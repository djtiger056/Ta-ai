...
class CerebellumEngine:
    ...
    def _load_config(self) -> CerebellumConfigData:
        ...
        cfg = CerebellumConfigData(
            ...
            proactive_chat=self._merge_dict(
                DEFAULT_PROACTIVE_CHAT,
                (
                    raw.get("proactive_chat")
                    if isinstance(raw.get("proactive_chat"), dict)
                    else (dict(global_config.proactive_chat_config) if isinstance(global_config.proactive_chat_config, dict) else {})
                ),
            ),
